...
<div class="container">
	<div class="sixteen columns">
		...
		<!-- Logo -->
		<div id="logo">
		<h1><a href="index.php"><img style="width:30%;height:30%;" src="images/logo2.png" alt="Akazi" /></a></h1>
		</div>
...
<div id="dashboard">
	...
	<div class="dashboard-content">
		...
		<?php
				...
				$resultUser = $conn->prepare("select * from applicants where app_id = '$app_id' ")or die(mysql_error());
				$resultUser->execute();
			...
			<form class="form-sample" method="POST" action="">
				...
				<div class="dashboard-list-box margin-top-0">
					...
					<div class="submit-page">
						...
						<div class="form">
							<h5> Email</h5>
							<input class="search-field" readonly name="email"  type="email"  value="<?php echo $email; ?>"/>
							<input class="search-field" name="app_id"  type="hidden"  value="<?php echo $app_id; ?>"/>
						</div>

						<!-- Title -->
						<div class="form">
							<h5>Mobile Number</h5>
							<input class="search-field" readonly name="mobile_no"  type="text" placeholder="" value="<?php echo $mobile_no; ?>"/>
						</div>

						<!-- Job Type -->
						<div class="form">
							<h5>Role : <?php echo $role; ?> </h5>
							<select  name="role" value="<?php echo $role; ?>" required class="chosen-select-no-single">
								<option value=""></option>
	...
	<!-- Content / End -->
			...
			$conn = new PDO('mysql:host=localhost;dbname=Akazi', 'gratty', 'test123456');
				...
				$conn ->query("UPDATE applicants set
				 role='$role'
				   
				 where app_id='$app_id' ")or die(mysql_error());
				...
				echo "<script>alert('Role Changed')</script>";
				echo "<script>window.open('dashboard-user.php','_self')</script>";
				}
			?>
